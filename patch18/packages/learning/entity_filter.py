...
def classify_entity(label: str, *, min_tokens: int = 1) -> EntityVerdict:
    ...
    # --- تنظيف السوابق -------------------------------------------
    # تنظيف الحدود قبل أي شيء: "، عن ابن ابي عمير" ظهرت كياناً في
    # المخرجات لأن الفاصلة كانت رمزاً مستقلاً في المقدمة، فلم تلتقطها
    # قائمةُ الأدوات ولا فحصُ الترقيم.
    while toks and not any(
        "\u0621" <= ch <= "\u064a" or ch.isalnum() for ch in toks[0]
    ):
        toks.pop(0)
    while toks and not any(
        "\u0621" <= ch <= "\u064a" or ch.isalnum() for ch in toks[-1]
    ):
        toks.pop()
    if not toks:
        return EntityVerdict(raw, EntityKind.REJECTED, False, "ترقيم خالص")

    core = _strip_trailing_function_words(_strip_leading_function_words(toks))
    # "بن" المعلَّقة في المقدمة تعني اسماً مبتوراً، لكن "ابن" فاتحة
    # نسب مشروعة: "ابن أبي عمير" و"ابن محبوب" أسماء كاملة. قشرها
    # يحوّلها إلى "أبي عمير" وهو خطأ.
    while core and _norm(core[0]) == "بن":
        core.pop(0)
