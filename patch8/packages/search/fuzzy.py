...
from sqlalchemy import REAL, cast, desc, func, select
...
class FuzzySearcher:
    ...
    def _prepare(self) -> None:
        ...
        # 1) محاولة الضبط
        #
        # التحويل إلى REAL إلزامي: set_limit معرّفة على real فقط، و
        # SQLAlchemy يرسل float كـ double precision، فيفشل الاستدعاء بـ
        #   function set_limit(double precision) does not exist
        # وحينها يعمل البحث على المسار الاحتياطي البطيء بلا أن يلاحظ أحد.
        try:
            with self.db.begin_nested():
                self.db.execute(select(func.set_limit(cast(self.threshold, REAL))))
