...
# حروف تقف كلمةً مستقلة أو سابقةً شرعية، فلا تُعدّ شظية.
# غيابها هو ما سمح بـ "ًا" + "من" -> "ًامن".
STANDALONE_LETTERS = {
    "\u0648", "\u0627", "\u0623", "\u0641", "\u0628",
    "\u0644", "\u0643", "\u064a", "\u0621", "\u0645", "\u0647",
}
    ...
    *,
    ...
    while i < len(tokens):
        ...
        if cur and nxt and _is_pure_arabic(cur) and _is_pure_arabic(nxt):
            ...
            # الحماية تُفحص على **الجانبين** لا على الأقصر وحده.
            #
            # العطب الذي يصلحه هذا الشرط: في "نقيّ ًا من الدنس" الشظيةُ
            # الأقصر هي "ًا" ومفتاحها "ا"، فلم تُفحص "من" المحمية إطلاقاً
            # فالتحمتا إلى "ًامن". وكذلك "مّ وأُ" صارت "مّوأُ".
            both_free = (
                cur_key not in PROTECTED_SHORT_WORDS
                and nxt_key not in PROTECTED_SHORT_WORDS
            )
